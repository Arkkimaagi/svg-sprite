...
function SVGShape(file, spriter) {
    ...
    this.meta = this.config.meta && this.id in this.config.meta ? this.config.meta[this.id] : (this.config.meta && relative in this.config.meta ? this.config.meta[relative] : {});
    this.align = Object.entries({ ...(this.config.align && this.config.align['*'] ? this.config.align['*'] : {}), ...(this.config.align && this.id in this.config.align ? this.config.align[this.id] : (this.config.align && relative in this.config.align ? this.config.align[relative] : {})) });
...
SVGShape.prototype._initSVG = function() {
    ...
    this.dom = new DOMParser({
        ...
        errorHandler(level, message) {
            const e = new Error(format('Invalid SVG file (%s)', message.replace(/\s+/g, ' ').split('\n').join(' ')));
            e.name = 'ArgumentError';
