...
module.exports = class SVGSpriterConfig {
            ...
            if (stat) {
                ...
                for (const a in align) {
                    if (isPlainObject(align[a]) && Object.keys(align[a]).length) {
                        ...
                        for (const [tmpl, tmplValue] of Object.entries(align[a])) {
                            const template = tmpl.length ? (tmpl.includes('%s') ? tmpl : `%s${tmpl}`) : '%s';
                            this.shape.align[path.join(path.dirname(a), path.basename(a, '.svg'))][template] = Math.max(0, Math.min(1, Number.parseFloat(tmplValue)));
                        }
                    }
                }
    ...
    filter(config = {}) {
        ...
        for (const [mode, value] of Object.entries(config)) {
            const configMode = isPlainObject(value) ? value : (value === true ? {} : null);
